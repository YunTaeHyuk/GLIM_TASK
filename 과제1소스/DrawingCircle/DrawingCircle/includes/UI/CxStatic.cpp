...
void CxStatic::SetFont(const CString& strFont,int nPointSize, int nWeight,
					   BOOL bRedraw /*= TRUE*/)
{
	memset(&m_lf, 0, sizeof(LOGFONT));       // Zero out the structure.

	_tcscpy(m_lf.lfFaceName,strFont);			// Set Font Familly
	m_lf.lfHeight = nPointSize;// Set Height
	m_lf.lfWeight = nWeight;					// Set Weight
...
void CxStatic::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	CMemDCa MemDC(pDC, &rect, m_bTransparentBk);
...
//	CMemDCa MemDC(&dc, &rc, m_bTransparentBk);
//
//	// ROUNDED_RECT
//	/*if (m_bRounded){
//	CBrush *oldbrush;
...
#define NMAP_STYLE(src, dest) if(!(dwStyle & (src))) dwFlags |= (dest)
	...
	if ( m_bAutoWrapping ){
		for (int i = 1; i <= csText.GetLength(); i++){
			...
			if (csText.GetAt(i-1) == '\r' || csText.GetAt(i-1) == '\n'){
				...
				asText.Add(s2);
				//Check a case of "\r\n" or "\n\r"
				if(csText.GetAt(i) == '\r' || csText.GetAt(i) == '\n'){
	...
	if (m_bAutoAdjustFont){
		while (nDiff < 0){
			...
			nDiff = pRect->bottom - pRect->top - (m_nFontSizeVar * nLine);
		}
		//pDCMem->SelectObject(&m_font); //TODO CHECK WITH AUTOADJUST
		//RedrawWindow();
	}
